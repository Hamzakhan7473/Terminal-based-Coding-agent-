...
class EnhancedUI:
    ...
    def render_file_content(self, file_path: str, highlight_lines: Optional[List[int]] = None) -> Panel:
        ...
        try:
            ...
            return Panel(
                ...
                title=f"📄 {file_path}",
                border_style="cyan",
                subtitle=f"[dim]{len(content)} chars, {content.count(chr(10)) + 1} lines[/dim]"
            )
    ...
    def render_status_bar(self, info: dict) -> Panel:
        ...
        status_text.append(f"Session: {info.get('session_id', 'N/A')[:8]}", style="dim")
        ...
        # Messages
        status_text.append("💬 ", style="cyan")
        status_text.append(f"{info.get('messages', 0)} messages", style="dim")
        status_text.append(" | ", style="dim")
        
        # Codebase
        if info.get('indexed_files'):
            status_text.append("🗂️  ", style="cyan")
            status_text.append(f"{info['indexed_files']} files indexed", style="dim")
    ...
    def render_code_preview_panel(self, code: str, language: str = "python") -> Panel:
        ...
        return Panel(
            ...
            border_style="green",
            subtitle="[dim]Generated by AI[/dim]"
        )
